#!/bin/bash
#
# S03_02_creeaza_tema.sh - Generator Variante Temă Seminar 3
...
usage() {
    cat << EOF
${BLUE}═══════════════════════════════════════════════════════════════════${NC}
       Generator Variante Temă - Seminar 3 Sisteme de Operare
${BLUE}═══════════════════════════════════════════════════════════════════${NC}
...
*Temă generată automat - Seminar 3 SO*
EOF
...
- [ ] Arhiva: \`tema_sem03_${name}.tar.gz\`
...
#!/bin/bash
#
# setup_tema.sh - Pregătire mediu pentru tema Sem 03
#
    ...
    while IFS=',' read -r name grupa email || [[ -n "$name" ]]; do
        ...
        if [[ $preview -eq 1 ]]; then
            ...
            if [[ $create_zip -eq 1 ]]; then
                ...
                log_info "  Arhivă creată: tema_sem03_$name.zip"
            fi
        fi
...
main() {
    ...
    echo ""
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════════${NC}"
    echo -e "     ${GREEN}Generator Variante Temă - Seminar 3 SO${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════════════════════${NC}"
    echo ""
