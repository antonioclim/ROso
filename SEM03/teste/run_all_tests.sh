...
# Utilizare: bash teste/run_all_tests.sh
# ═══════════════════════════════════════════════════════════════════════════════
...
# Culori (dezactivate dacă nu e terminal)
if [[ -t 1 ]]; then
    RED='\033[0;31m'
    GREEN='\033[0;32m'
    YELLOW='\033[0;33m'
...
print_header() {
    echo -e "${BLUE}"
    echo "═══════════════════════════════════════════════════════════════"
    echo "        TESTE SEMINAR 03: System Administration"
    echo "═══════════════════════════════════════════════════════════════"
    echo -e "${NC}"
}
...
test_structure() {
    print_section "Structură Fișiere"
    
    # Fișiere root obligatorii
...
# ─────────────────────────────────────────────────────────────────────────────
# TESTE: VALIDARE YAML/JSON
# ─────────────────────────────────────────────────────────────────────────────

test_data_files() {
    print_section "Validare Date (YAML/JSON)"
    ...
    if [[ -f "$PROJECT_ROOT/formative/quiz.yaml" ]]; then
        ...
        run_test "quiz.yaml has questions" \
            "python3 -c \"import yaml; q=yaml.safe_load(open('$PROJECT_ROOT/formative/quiz.yaml')); assert 'intrebari' in q and len(q['intrebari']) > 0\""
    else
...
# ─────────────────────────────────────────────────────────────────────────────
# TESTE: CONSISTENȚĂ NAMING
# ─────────────────────────────────────────────────────────────────────────────

test_naming_consistency() {
    print_section "Consistență Naming"
    
    # Verifică absența referințelor greșite la "Seminar 3"
    ...
    # Verifică absența email-urilor de contact
...
test_code_quality() {
    print_section "Calitate Cod"
    
    # Verifică shebang în scripturi bash
    ...
    # Verifică set -euo pipefail în scripturi principale
    local main_scripts=(
        "scripts/bash/S03_01_setup_seminar.sh"
        "scripts/bash/S03_03_validator.sh"
    )
...
test_ai_patterns() {
    print_section "Verificare Patterns AI"
    
    # Cuvinte-semnal AI comune
    ...
    # Verifică absența "Happy scripting!"
...
main() {
    ...
    # Rulează toate categoriile de teste
    test_structure
    test_bash_syntax
    test_python_syntax
    test_data_files
    ...
    # Exit code bazat pe teste failed
