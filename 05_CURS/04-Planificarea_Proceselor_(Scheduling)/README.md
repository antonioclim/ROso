...
1. **[REMEMBER]** Enumeră cele 4 algoritmi de scheduling studiați și caracteristica principală a fiecăruia.
2. **[UNDERSTAND]** Explică conceptul de "quantum" în Round Robin. Ce se întâmplă dacă quantum-ul este prea mic? Dar prea mare?
3. **[ANALYSE]** Compară FCFS cu SJF din perspectiva timpului mediu de așteptare. În ce situații SJF poate fi problematic (starvation)?

### Mini-provocare (opțional)

Folosind comanda `nice`, rulează două procese CPU-intensive cu priorități diferite și observă diferența în alocarea CPU.
...
- **CFS (Completely Fair Scheduler)**: Scheduler-ul real din Linux, bazat pe red-black trees și virtual runtime.
- **Real-time scheduling**: SCHED_FIFO, SCHED_RR cu priorități RT (1-99).
- **CPU affinity**: Pinning procese pe anumite core-uri pentru cache locality.
- **NUMA scheduling**: Plasarea proceselor aproape de memoria lor pe sisteme multi-socket.

### Greșeli frecvente de evitat

1. **Confuzia nice vs priority**: `nice` influențează prioritatea, dar relația nu e liniară în CFS.
2. **Ignorarea I/O wait**: Un proces "CPU-bound" poate deveni "I/O-bound" sub load diferit.
3. **Over-engineering quantum**: Valorile default (1-10ms) sunt bune pentru majoritatea cazurilor.

### Întrebări rămase deschise

- Cum ar trebui să planifice SO-ul workload-uri AI (lungi, predictibile) vs interactive?
- Poate ML să îmbunătățească deciziile scheduler-ului în timp real?

## Privire înainte

**Săptămâna 5: Fire de Execuție (Threads)** — De la procese grele (heavyweight) trecem la fire de execuție ușoare (lightweight). Thread-urile partajează spațiul de adrese, ceea ce aduce atât avantaje (comunicare rapidă) cât și provocări (sincronizare).

**Pregătire recomandată:**
- Citește OSTEP Capitolul 26 (Concurrency and Threads)
- Experimentează cu `top -H` pentru a vedea thread-urile

## Rezumat Vizual

```
┌─────────────────────────────────────────────────────────────────┐
│                    SĂPTĂMÂNA 4: SCHEDULING — RECAP              │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  OBIECTIV SCHEDULER: Decide CINE rulează și CÂT                │
│                                                                 │
│  ALGORITMI NON-PREEMPTIVI                                       │
│  ├── FCFS: primul venit, primul servit (simplu, convoy)        │
│  └── SJF: cel mai scurt job primul (optimal, starvation)       │
│                                                                 │
│  ALGORITMI PREEMPTIVI                                           │
│  ├── Round Robin: quantum fix, fairness (overhead)             │
│  ├── SRTF: SJF + preemption                                    │
│  └── MLFQ: cozi multiple, prioritate dinamică                  │
│                                                                 │
│  METRICI                                                        │
...
│                                                                 │
│  💡 TAKEAWAY: Nu există algoritm perfect — totul e compromis   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
