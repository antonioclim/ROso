...
1. **[REMEMBER]** Enumeră cele 4 condiții Coffman necesare pentru deadlock. Pentru fiecare, dă un exemplu concret.
2. **[UNDERSTAND]** Explică algoritmul Banker. Cum determină dacă o stare este "safe"? Ce înseamnă "safe sequence"?
3. **[ANALYSE]** Compară cele 3 strategii de gestionare a deadlock-ului: prevenire, evitare, detectare+recuperare. Care sunt compromisurile fiecăreia?

### Mini-provocare (opțional)

Desenează graful de alocare resurse pentru problema filozofilor la masă (5 filozofi) și identifică ciclul de deadlock.
...
- **Livelock**: Procesele nu sunt blocate dar nu progresează (ex: două persoane care se feresc în aceeași direcție).
- **Distributed deadlock**: Mult mai greu de detectat; necesită algoritmi ca Chandy-Misra-Haas.
- **Resource starvation**: Diferit de deadlock; un proces nu primește niciodată resursa dorită.

### Greșeli frecvente de evitat

1. **Presupunerea că try_lock rezolvă tot**: Poate duce la livelock dacă toți fac retry simultan.
2. **Ignorarea timeout-urilor**: În producție, folosește locks cu timeout și retry logic.
3. **Detectare fără recovery**: Detectezi deadlock dar nu știi cum să-l rezolvi (care proces să omori?).

### Întrebări rămase deschise

- Cum detectează bazele de date distribuite (CockroachDB, Spanner) deadlock-ul cross-node?
- Poate analiza statică (la compile time) să elimine posibilitatea de deadlock?

## Privire înainte

**Săptămâna 9: Gestiunea Memoriei (Partea 1)** — De la procese și sincronizare trecem la gestiunea memoriei. Vom studia cum SO-ul alocă memorie proceselor, paginarea, segmentarea și conceptul de spațiu de adrese virtual.

**Pregătire recomandată:**
- Revizuiește structura spațiului de adrese al unui proces
- Rulează `cat /proc/self/maps` pentru a vedea mapping-urile

## Rezumat Vizual

```
┌─────────────────────────────────────────────────────────────────┐
│                    SĂPTĂMÂNA 8: DEADLOCK — RECAP                │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  DEADLOCK = Așteptare circulară permanentă                     │
│                                                                 │
│  CELE 4 CONDIȚII COFFMAN (toate necesare)                       │
│  ├── 1. Mutual Exclusion: resursa nu e partajabilă             │
│  ├── 2. Hold and Wait: deții și aștepți                        │
│  ├── 3. No Preemption: nu se poate lua forțat                  │
│  └── 4. Circular Wait: A→B→C→...→A                             │
│                                                                 │
│  STRATEGII                                                      │
│  ├── PREVENIRE: elimină una din cele 4 condiții                │
│  ├── EVITARE: Banker's Algorithm (safe state)                  │
│  ├── DETECTARE: găsește cicluri în RAG                         │
│  └── RECUPERARE: kill proces sau preempt resurse               │
│                                                                 │
│  PROBLEMA FILOZOFILOR                                           │
│  ├── 5 filozofi, 5 furculițe, masă rotundă                     │
│  └── Soluții: asimetrie, timeout, arbitru central              │
│                                                                 │
│  💡 TAKEAWAY: Cel mai sigur = prevenire prin ordonare resurse  │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
