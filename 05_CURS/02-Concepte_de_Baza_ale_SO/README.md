...
1. **[REMEMBER]** Ce este un system call și care este rolul său în arhitectura SO?
2. **[UNDERSTAND]** Explică diferența dintre modul utilizator (user mode) și modul kernel (kernel mode). De ce este necesară această separare?
3. **[ANALYSE]** Analizează ce se întâmplă când o aplicație execută `open("/etc/passwd", O_RDONLY)`. Care sunt pașii parcurși de la user space la kernel și înapoi?

### Mini-provocare (opțional)

Rulează `strace ls -la` și identifică cele mai frecvente 3 tipuri de system calls. Explică rolul fiecăruia.
...
- **vDSO (Virtual Dynamic Shared Object)**: Mecanism Linux care permite anumite syscalls (ex: `gettimeofday`) să ruleze în user space, evitând overhead-ul kernel transition. Optimizare avansată.
- **io_uring**: Interfață modernă pentru I/O asincron cu performanțe superioare syscall-urilor tradiționale. Introdus în Linux 5.1.
- **Seccomp-BPF**: Filtrare syscalls pentru sandboxing (folosit de containere, browsere).

### Greșeli frecvente de evitat

1. **Confuzia API vs Syscall**: `printf()` NU este syscall; este funcție de bibliotecă care *eventual* apelează `write()`.
2. **Ignorarea errno**: După un syscall eșuat, `errno` conține codul erorii. Verifică întotdeauna return value-ul.
3. **Presupunerea că syscall-urile sunt atomice**: Multe nu sunt (ex: `write()` poate scrie parțial).

### Întrebări rămase deschise

- Cum va evolua interfața syscall odată cu hardware-ul heterogen (CPU+GPU+NPU)?
- Poate WebAssembly System Interface (WASI) să înlocuiască syscall-urile tradiționale pentru aplicații portabile?

## Privire înainte

**Săptămâna 3: Procese (PCB + fork)** — Vom vedea cum sistemul de operare creează și gestionează procesele, folosind conceptele de system calls învățate astăzi. Apelul `fork()` este piatra de temelie a multitasking-ului în UNIX.

**Pregătire recomandată:**
- Experimentează cu `ps aux` și `htop` pentru a vedea procesele în acțiune
- Citește OSTEP Capitolele 4-5 (Processes)

## Rezumat Vizual

```
┌─────────────────────────────────────────────────────────────────┐
│                    SĂPTĂMÂNA 2: CONCEPTE DE BAZĂ — RECAP        │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  SYSTEM CALLS (APELURI DE SISTEM)                               │
│  ├── Interfața user space ↔ kernel space                       │
│  ├── Mecanismul: trap/interrupt → kernel → return              │
│  └── Categorii: procese, fișiere, dispozitive, informații      │
│                                                                 │
│  MODURI DE EXECUȚIE                                             │
│  ├── User Mode: acces limitat, aplicații normale                │
│  ├── Kernel Mode: acces complet, cod privilegiat                │
│  └── Tranziția: syscall/trap → kernel → return                 │
│                                                                 │
│  STRUCTURA SO                                                   │
│  ├── Monolitic: totul în kernel (Linux)                        │
│  ├── Microkernel: servicii în user space (Minix)               │
│  └── Hibrid: compromis (Windows NT, macOS)                     │
│                                                                 │
│  💡 TAKEAWAY: Orice acțiune "utilă" trece prin system calls    │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
