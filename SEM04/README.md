...
## ⚠️ Probleme cunoscute și limitări

### Specifice platformei

1. **macOS sed vs GNU sed:** macOS folosește BSD sed care are sintaxă diferită pentru `-i`.  
   - GNU: `sed -i 's/a/b/' file`  
   - BSD: `sed -i '' 's/a/b/' file`

2. **`grep -P` pe macOS:** `grep -P` nu este suportat implicit pe macOS.  
   - Folosiți `grep -E` (recomandat) sau instalați `ggrep` (GNU grep).

3. **Disponibilitatea `bc`:** unele imagini minimale nu includ `bc`.  
   - Folosiți `awk` sau `python3` pentru calcule simple, dacă este necesar.

### Dependințe

4. **/usr/share/dict/words:** unele distribuții nu au dicționarul de cuvinte.  
   - Scripturile au fallback, dar veți obține „wordX” în loc de cuvinte reale.

5. **`shuf`:** `shuf` nu există pe macOS în mod implicit.  
   - Instalați GNU coreutils:
   ```bash
   brew install coreutils
   ```
   - Folosiți `gshuf` în loc de `shuf`.

### Structura fișierelor

6. **Line endings:** dacă copiați fișiere pe Windows, `
` poate strica regex‑urile.  
   - Convertiți cu:
   ```bash
...
Dacă găsiți un bug sau o inconsistență în materiale:

1. Verificați mai întâi `docs/` și `qa_artifacts/` pentru note existente
2. Verificați dacă problema este specifică OS‑ului dumneavoastră (macOS vs Linux)
3. Deschideți un issue (sau raportați instructorului) cu:
   - ce ați rulat
   - output obținut
   - output așteptat
   - versiunea OS + shell
...
Aceste materiale sunt destinate uzului educațional în cadrul ASE București (CSIE).

Dacă reutilizați materialul, păstrați atribuirile (autor, sursă, licență) și nu eliminați identificatorii de versiune.
