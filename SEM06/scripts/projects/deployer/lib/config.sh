...
# ==============================================================================
# Parsare configurare, argumente CLI, validare
...
# Se setează din fișier configurare sau CLI
...
# Acțiune (deploy, rollback, status, list)
ACTION="deploy"
...
# Locații fișiere configurare (în ordine de prioritate)
CONFIG_LOCATIONS=(
    "./etc/deployer.conf"
...
# Încarcă fișier de configurare
# Args: $1 = path fișier (opțional)
load_config_file() {
    local config_file=""
    
    # Dacă e specificat explicit
    if [[ -n "${1:-}" ]]; then
        config_file="$1"
...
validate_config() {
    ...
    # Validare directoare (cu creare automată în mod non-dry-run)
    ...
    # Validare Docker dacă e necesar
    ...
    # Verificare că avem ceva de deployat
...
# Inițializare completă configurare
# Args: $@ = argumentele CLI
init_config() {
    # Încarcă configurația default din fișier (dacă există)
    load_config_file || true
    
    # Parsează argumentele CLI (suprascriu fișierul)
    parse_args "$@"
    
    # Setează log level din configurare
    ...
    # Setează log file dacă nu e specificat
    ...
    # Afișează configurația în mod verbose
