# TC1b - Configurarea Shell-ului (Variabile)
...
│                    VARIABILE BASH                           │
├─────────────────────────────────────────────────────────────┤
│  LOCALE           │  MEDIU (Environment)  │  SPECIALE      │
│  - doar în shell  │  - moștenite de       │  - setate de   │
│    curent         │    subprocese         │    shell       │
│  - VAR=val        │  - export VAR=val     │  - $?, $$, $!  │
└─────────────────────────────────────────────────────────────┘
```
...
```bash
# Definire variabilă locală (fără spații în jurul =)
NUME="Ion Popescu"
VARSTA=25
ACTIV=true
...
# Ștergere variabilă
unset NUME
echo $NUME    # nu afișează nimic
```

### 1.3 Variabile de Mediu (Environment)
...
# Vizualizează o variabilă specifică
echo $PATH
printenv HOME

# Creează variabilă de mediu
export EDITOR="vim"
export PROJECT_DIR="/home/student/proiect"

# Diferența: locală vs mediu
LOCAL_VAR="local"
export ENV_VAR="mediu"

bash -c 'echo "Local: $LOCAL_VAR"'    # nu afișează nimic
bash -c 'echo "Mediu: $ENV_VAR"'      # afișează "mediu"
```

### 1.4 Variabile Speciale

| Variabilă | Descriere | Exemplu |
|-----------|-----------|---------|
| `$?` | Exit code ultima comandă | `0` = succes |
| `$$` | PID-ul shell-ului curent | `12345` |
| `$!` | PID-ul ultimului proces background | `12346` |
| `$0` | Numele script-ului | `./script.sh` |
| `$1, $2...` | Parametrii poziționali | primul, al doilea... |
| `$#` | Numărul de parametri | `3` |
| `$@` | Toți parametrii (ca listă) | `"a" "b" "c"` |
| `$*` | Toți parametrii (ca string) | `"a b c"` |
