...
CONFIG_FILE = os.path.expanduser("~/.homework_recorder_config.json")
...
def install_python_packages():
    ...
    if missing:
        ...
        pip_cmd = [sys.executable, '-m', 'pip', 'install', '--quiet', '--user']
        ...
        try:
            ...
            try:
                ...
                if pip_major_version >= 23:
                    ...
                    apt_packages = ['python3-rich']  # questionary nu e în apt, dar rich da
                    subprocess.run(['sudo', 'apt', 'install', '-y'] + apt_packages, check=False)
                    # Reîncearcă pip pentru questionary
...
MATRIX_GREEN = "#00ff41"
MATRIX_DARK_GREEN = "#00cc44"
MATRIX_BRIGHT = "#00ff00"
MATRIX_DIM = "#00aa33"
MATRIX_CYAN = "#00ffff"
...
def matrix_rain(duration=1.5, width=None):
...
def typing_effect(text, style=MATRIX_GREEN, delay=0.02):
...
def spinner_task(description, task_func, *args, **kwargs):
...
def progress_bar(description, total, update_func):
...
def validate_surname(text):
    """Validează numele de familie (doar litere și cratimă)."""
    if not text:
        return False
...
def collect_student_data():
    ...
    config = load_config()
    prev_surname = config.get('surname', '')
    prev_firstname = config.get('firstname', '')
    prev_group = config.get('group', '')
    ...
    # Nume de familie
    while True:
        default_hint = f" [{prev_surname}]" if prev_surname else ""
        surname = questionary.text(
...
def start_recording(filepath, data):
...
def generate_signature(filepath, data):
    ...
    def do_sign():
        ...
        data_to_sign = f"{data['surname']}+{data['firstname']} {data['group']} {file_size} {current_date} {current_time} {system_user} {absolute_path}"
        ...
        try:
            ...
            encrypted_b64 = base64.b64encode(process.stdout).decode()
    ...
    # Execută cu spinner
    signed_data = spinner_task("Se generează semnătura RSA...", do_sign)
...
[{MATRIX_YELLOW}]  NU modifica fișierul .cast înainte de trimitere![/]""")),
...
def show_summary(data, filepath, upload_success):
    ...
    status = f"[{MATRIX_BRIGHT}]✓ Încărcat[/]" if upload_success else f"[{MATRIX_YELLOW}]⚠ Doar local[/]"
