...
| Seminar | Conținut |
|---------|----------|
| Seminar 1 | Navigare în sistem de fișiere, variabile, globbing |
| Seminar 2 | Operatori, redirecționare, pipe-uri, filtre, bucle de bază |
| Seminar 3 | `find`, `xargs`, scripturi cu argumente, permisiuni, cron |
| Seminar 4 | Expresii regulate, `grep`, `sed`, `awk`, editorul nano |

Ce introduce acest seminar:

- Funcții avansate: variabile locale cu `local`, valori de întoarcere, nameref, recursivitate
- Arrays: indexate (indexare de la 0) și asociative (hash maps cu `declare -A`)
- Stabilitate: `set -euo pipefail`, IFS sigur, verificări defensive
- Gestionarea erorilor: trap EXIT/ERR/INT/TERM, modele de curățare (cleanup)
- Jurnalizare și depanare: sistem profesional de logging cu nivele, tehnici de debug
...
- Creeze funcții cu variabile locale și mecanisme de valori de întoarcere
...
- Gestionarea erorilor - Script-urile nu mai "mor" silențios
- Jurnalizare - Poți depana probleme fără să fii prezent
...
| Jurnalizare | Vezi ce face scriptul | Depanareging post-mortem |
...
│
├── resources/                               # Materiale suplimentare
...
# 5. Auto-evaluare
cat docs/S05_10_SELF_ASSESSMENT_REFLECTION.md | less
```
