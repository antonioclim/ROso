...
"""
═══════════════════════════════════════════════════════════════════════════════
📊 S02_03_report_generator.py - Generator Rapoarte și Statistici
═══════════════════════════════════════════════════════════════════════════════

DESCRIERE:
    Agregă rezultatele evaluărilor (autograder) și generează rapoarte:
    - Statistici per grupă și per student
    - Identificare probleme comune
    - Rapoarte HTML interactive cu grafice
    - Export CSV pentru importare în Excel/Moodle
...
@dataclass
class StudentResult:
    """Rezultatul unui student la o evaluare."""
    student_name: str
    student_group: str
    score: float
    max_score: float
...
@dataclass 
class OverallReport:
    """Raport general pentru toate grupele."""
    generated_at: str
    total_students: int
...
# 
# SECȚIUNEA 2: CITIREA REZULTATELOR
# 

class ResultLoader:
    """Încarcă rezultatele din diverse formate."""
    
    @staticmethod
    def load_json(filepath: str) -> StudentResult:
        """Încarcă un rezultat din fișier JSON."""
    ...
    def load_directory(dirpath: str) -> List[StudentResult]:
        """Încarcă toate rezultatele dintr-un director."""
        results = []
    ...
    def load_csv(filepath: str) -> List[StudentResult]:
        """Încarcă rezultatele dintr-un CSV."""
        results = []
...
class StatisticsAnalyzer:
    ...
    def generate_overall_report(self) -> OverallReport:
        ...
        # Performanță per categorie globală
...
# 
# SECȚIUNEA 4: EXPORTATORI DE RAPOARTE
# 

class ReportExporter:
    """Exportă rapoarte în diverse formate."""
    
    @staticmethod
    def to_csv(report: OverallReport, filepath: str):
        """Exportă rezultatele individuale în CSV."""
        with open(filepath, 'w', encoding='utf-8', newline='') as f:
            ...
            # Adaugă coloane pentru categorii
            all_categories = set()
    ...
    def to_summary_txt(report: OverallReport) -> str:
        """Generează un rezumat text."""
        lines = []
        lines.append("=" * 70)
        lines.append("  RAPORT SUMAR - Evaluare Seminarul 3-4")
        lines.append("  Generat: " + report.generated_at[:19].replace('T', ' '))
        lines.append("=" * 70)
        lines.append("")
        ...
        lines.append("-" * 40)
        lines.append(f"  Total studenți evaluați: {report.total_students}")
        lines.append(f"  Media generală: {statistics.mean(all_scores):.1f}%")
        lines.append(f"  Mediana: {statistics.median(all_scores):.1f}%")
        if len(all_scores) > 1:
            lines.append(f"  Deviația standard: {statistics.stdev(all_scores):.1f}")
        lines.append(f"  Scor minim: {min(all_scores):.1f}%")
        lines.append(f"  Scor maxim: {max(all_scores):.1f}%")
        lines.append("")
        
        # Distribuție note
        lines.append("📈 DISTRIBUȚIE NOTE")
        lines.append("-" * 40)
        ...
        lines.append(f"  Rata de promovare (≥50%): {pass_rate:.1f}%")
        lines.append("")
        
        # Statistici per grupă
        lines.append("👥 STATISTICI PER GRUPĂ")
        lines.append("-" * 40)
        lines.append(f"  {'Grupa':<10} {'N':>5} {'Media':>8} {'Med.':>8} {'Min':>6} {'Max':>6} {'Pass%':>8}")
        lines.append("  " + "-" * 55)
        ...
        # Performanță per categorie
        if report.category_performance:
            lines.append("📚 PERFORMANȚĂ PER CATEGORIE")
            lines.append("-" * 40)
            for cat, data in sorted(report.category_performance.items()):
                lines.append(f"  {cat:<20} Media: {data['mean']:>5.1f}%  "
                            f"(Min: {data['min']:.0f}% - Max: {data['max']:.0f}%)")
            lines.append("")
        
        # Top probleme
        if report.top_issues:
            lines.append("⚠️ PROBLEME FRECVENTE")
            lines.append("-" * 40)
    ...
    def to_html(report: OverallReport) -> str:
        """Generează un raport HTML interactiv cu grafice."""
        
        # Pregătire date pentru grafice
        all_scores = [r.percentage for r in report.all_results]
        
        # Date pentru histogramă
        ...
        # Date per grupă pentru bar chart
        group_data = {
            'labels': [],
            'means': [],
            'pass_rates': []
...
<head>
    ...
    <title>Raport Evaluare - Seminarul 3-4</title>
...
<body>
    <div class="container">
        <div class="header">
            <h1>📊 Raport Evaluare - Seminarul 3-4</h1>
            <div class="meta">
                <span>📅 Generat: {report.generated_at[:19].replace('T', ' ')}</span>
                <span>👥 Total studenți: {report.total_students}</span>
                <span>📚 Grupe evaluate: {len(report.groups)}</span>
            </div>
        </div>
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="label">Studenți Evaluați</div>
            </div>
            <div class="stat-card">
            ...
            <div class="stat-card">
                ...
                <div class="label">Rata Promovare</div>
            </div>
        </div>
        
        <div class="charts-grid">
            <div class="chart-container">
                <h3>📈 Distribuție Note</h3>
                <div class="chart-wrapper">
                    <canvas id="gradeChart"></canvas>
                </div>
            </div>
            <div class="chart-container">
                <h3>📊 Histogramă Scoruri</h3>
                <div class="chart-wrapper">
                    <canvas id="histChart"></canvas>
                </div>
            </div>
            <div class="chart-container">
                <h3>👥 Comparație Grupe</h3>
                <div class="chart-wrapper">
...
"""
        ...
        <div class="section">
            ...
            <input type="text" class="search-box" id="searchBox" 
                   placeholder="Caută student sau grupă..." onkeyup="filterTable()">
...
"""
    ...
    def to_moodle_csv(report: OverallReport, filepath: str, 
                      assignment_name: str = "Seminar 3-4"):
        """Exportă în format compatibil Moodle Gradebook."""
        with open(filepath, 'w', encoding='utf-8', newline='') as f:
            writer = csv.writer(f)
            ...
            for r in report.all_results:
                # Generează email fictiv bazat pe nume
                email = f"{r.student_name.lower().replace(' ', '.')}@student.ase.ro"
                feedback = f"Scor: {r.score}/{r.max_score}. " + "; ".join(r.issues[:3])
                writer.writerow([email, r.student_group, r.percentage, feedback])

# 
# SECȚIUNEA 5: COMPARAȚIE ÎNTRE EVALUĂRI
# 

class ComparisonAnalyzer:
    """Compară rezultate între două evaluări (progres)."""
    ...
    def analyze_progress(self) -> Dict[str, Any]:
        """Analizează progresul studenților."""
...
╔═══════════════════════════════════════════════════════════════════════════════╗
║                                                                               ║
║   📊  REPORT GENERATOR - Seminarul 3-4                                        ║
║       Agregare Rezultate și Statistici                                        ║
║                                                                               ║
...
def main():
    parser = argparse.ArgumentParser(
        description='Generator de rapoarte și statistici pentru evaluări',
...
Exemple de utilizare:
  ...
  %(prog)s --compare ./week1/ ./week2/ --output progress.html
    ...
    parser.add_argument('--input', '-i', required=True,
                        help='Fișier JSON/CSV sau director cu rezultate')
    parser.add_argument('--format', '-f',
                        choices=['txt', 'html', 'csv', 'moodle', 'all'],
                        default='txt',
                        help='Formatul raportului (default: txt)')
    parser.add_argument('--output', '-o',
                        help='Fișierul sau directorul de output')
    parser.add_argument('--compare', '-c', nargs=2, metavar=('BEFORE', 'AFTER'),
                        help='Compară două seturi de rezultate')
    parser.add_argument('--filter-group', '-g',
                        help='Filtrează doar o anumită grupă')
    parser.add_argument('--min-score', type=float,
                        help='Filtrează studenții cu scor minim')
    parser.add_argument('--quiet', '-q', action='store_true',
                        help='Mod silențios (fără output în terminal)')
    ...
    if input_path.is_dir():
        ...
        print(f"{Colors.RED}❌ Format necunoscut: {input_path}{Colors.ENDC}")
        return 1
    
    if not results:
        print(f"{Colors.RED}❌ Nu s-au găsit rezultate!{Colors.ENDC}")
        return 1
    
    if not args.quiet:
        print(f"{Colors.GREEN}✓ Încărcate {len(results)} rezultate{Colors.ENDC}")
    
    # Filtrare opțională
    if args.filter_group:
        results = [r for r in results if r.student_group == args.filter_group]
        if not args.quiet:
            print(f"  Filtrate pentru grupa {args.filter_group}: {len(results)} rezultate")
    
    if args.min_score:
        results = [r for r in results if r.percentage >= args.min_score]
        if not args.quiet:
            print(f"  Filtrate pentru scor minim {args.min_score}%: {len(results)} rezultate")
    ...
    if args.compare:
        ...
        if not args.quiet:
            print(f"\n{Colors.BOLD}📈 ANALIZĂ PROGRES{Colors.ENDC}")
            print(f"  Studenți comuni: {progress['common_students']}")
            print(f"  Îmbunătățiri: {len(progress['improvements'])} "
                  f"(medie: +{progress['avg_improvement']:.1f}%)")
            print(f"  Regresii: {len(progress['regressions'])} "
                  f"(medie: {progress['avg_regression']:.1f}%)")
            print(f"  Nemodificați: {len(progress['unchanged'])}")
            
            if progress['improvements']:
                print(f"\n  {Colors.GREEN}Top îmbunătățiri:{Colors.ENDC}")
    ...
    if output_dir.suffix:  # Este un fișier
    ...
    for fmt in formats_to_export:
        if fmt == 'txt':
            ...
            if not args.quiet:
                print(f"\n{content}")
                print(f"\n{Colors.GREEN}✓ Salvat: {filepath}{Colors.ENDC}")
            ...
            if not args.quiet:
                print(f"{Colors.GREEN}✓ Salvat raport HTML: {filepath}{Colors.ENDC}")
            ...
            if not args.quiet:
                print(f"{Colors.GREEN}✓ Salvat CSV: {filepath}{Colors.ENDC}")
            ...
            if not args.quiet:
                print(f"{Colors.GREEN}✓ Salvat Moodle CSV: {filepath}{Colors.ENDC}")
