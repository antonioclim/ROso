...
Note instructor:
- Exit code de la `mkdir` când directorul există = non-zero
...
| Distractor | Misconceptie vizată |
|------------|---------------------|
| A | Crede că pipe-ul transmite exit code-ul primei comenzi |
...
Note instructor:
- `>/dev/null` și `2>/dev/null` nu afectează exit code-ul
