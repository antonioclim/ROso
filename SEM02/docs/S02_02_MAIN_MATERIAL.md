...
| O6 | Depana scripturi cu probleme comune (subshell, brace expansion) | Analiză | Exercițiu LLM |
...
Operatorul `&&` (AND logic) execută comanda următoare DOAR dacă comanda precedentă a REUȘIT (cod de ieșire 0).
...
Operatorul `||` (OR logic) execută comanda următoare DOAR dacă comanda precedentă a EȘUAT (cod de ieșire non-zero).
...
🎯 SUBGOAL 1.8.1: Înțelege sistemul de coduri de ieșire

Fiecare comandă Unix returnează un cod de ieșire (cod de ieșire) - un număr între 0 și 255.
...
Verificarea cod de ieșire-ului:
```bash
ls /home
echo $?  # 0 (succes)
...
🎯 SUBGOAL 1.8.2: Folosește coduri de ieșire în scripturi
...
descriptor de fișier (fd): Un număr care identifică un canal I/O.
